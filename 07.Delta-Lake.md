...
```
Tamén se pode (OPCIÓN RECOMENDADA) facer a configuración permanente en `spark-defaults.conf`:

```bash
# JAR de Delta como dependency -> Engadir a outras jar.packages (se os hai) separado por ";"
spark.jars.packages io.delta:delta-spark_2.12:3.1.0

# SQL extensions -> OLLO ao convivir con iceberg.
spark.sql.extensions io.delta.sql.DeltaSparkSessionExtension

# Delta no catálogo por defecto
